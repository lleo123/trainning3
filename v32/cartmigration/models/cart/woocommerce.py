...
class LeCartWoocommerce(LeCartWordpress):
	...
	def get_categories_ext_export(self, categories):
		...
		if self._notice['src']['support']['wpml']:
			categories_ext_queries['icl_translations'] = {
				'type': 'select',
				'query': "SELECT * FROM _DBPRF_icl_translations WHERE element_type = 'tax_product_cat' andelement_id IN " + self.list_to_in_condition(
					category_ids)
			}
	...
	def category_import(self, convert, category, categories_ext):
		...
		self.import_category_data_connector(term_query, True, convert['id'])
		if not term_id:
			return response_warning('term ' + to_str(convert['id']) + ' import false.')

		# if not term_taxonomy_import:
		# 	return response_warning('manufacturer ' + to_str(convert['id']) + ' import false.')
		# self.insert_map(self.TYPE_MANUFACTURER, convert['id'], term_taxonomy_import, convert['code'])
		# thumbnail_id = False
		# cate_image = ''
		# if convert['thumb_image']['url'] or convert['thumb_image']['path']:
		...
		meta_query = self.create_insert_query_connector('termmeta', meta_insert)
		self.import_data_connector(meta_query, 'category')
		self.insert_map(self.TYPE_CATEGORY, convert['id'], term_id, convert['code'])
	...
	def product_import(self, convert, product, products_ext):
		...
		# 	# 'weight': convert['weight'] if convert['weight'] else '',
		# 	# 'length': convert['length'] if convert['length'] else '',
		# 	# 'width': convert['width'] if convert['width'] else '',
		# 	# 'height': convert['height'] if convert['height'] else '',
		# 	'sku': convert['sku'],
		# 	'virtual':'1',
		# 	'downloadable':'0',
		...
		product_post_data={
			...
			'post_parent':'0',
			# 'guid':'http://localhost/wordpress/wp-content/uploads/2021/01/'+convert['thumb_image']['path'],
			'guid':'',
			'menu_order':'0',
			'post_type':'product',
			'post_mime_type':'',
		...
		if self._notice['config']['pre_prd']:
			product_post_data['ID'] = convert['id']
		product_post_query = self.create_insert_query_connector("posts",product_post_data)
		product_id = self.import_product_data_connector(product_post_query, True, convert['id'])
		if product_id:
	...
	def after_product_import(self, product_id, convert, product, products_ext):
		...
		term_taxonomy_data_query = self.create_insert_query_connector('term_taxonomy', term_taxonomy_data)
		term_taxonomy_id = self.import_category_data_connector(term_taxonomy_data_query, True, convert['id'])
		...
		update_term = self.create_update_query_connector('term_taxonomy', data_cate, where_id)
		self.import_data_connector(update_term, 'term')
		category_data = {
						'object_id': product_id,
						'term_taxonomy_id': term_taxonomy_id,
						'term_order': 0
						}
		cate_rela = self.create_insert_query_connector("term_relationships", category_data)
		self.import_category_data_connector(cate_rela,True,convert['id'])

		products_posts_image = {
		...
		products_term_query_image = self.create_insert_query_connector('posts', products_posts_image)
		prd_id_image = self.import_product_data_connector(products_term_query_image, True, convert['id'])
		thumb = product['products_image']
		products_posts_image_expand = {
			'guid': '',
		}
		where_id = {
			'ID': prd_id_image
		...
		if thumb[0:4] != 'http':
			products_posts_image_expand['guid'] = self._notice['target']['cart_url'] + '/wp-content/uploads/2021/01/' + \
												  product['products_image']
		else:
			products_posts_image_expand['guid'] = product['products_image']
		update_query_image = self.create_update_query_connector('posts', products_posts_image_expand, where_id)
		self.import_data_connector(update_query_image, 'guid')
		data_meta_post = {
			'_thumbnail_id': prd_id_image,
		}
		data_meta_post_img = {
			'_wp_attached_file': '2021/01/' + product['products_image'],
		}
		data_meta = {
			'post_id': '',
			'meta_key': '',
		...
		if prd_id_image:
			for key, value in data_meta_post.items():
				data_meta['post_id'] = product_id
				data_meta['meta_key'] = key
				data_meta['meta_value'] = value
				self.import_product_data_connector(self.create_insert_query_connector('postmeta', data_meta), True,
												   convert['id'])
			for keys, values in data_meta_post_img.items():
				data_meta['post_id'] = prd_id_image
				data_meta['meta_key'] = keys
				data_meta['meta_value'] = values
				self.import_product_data_connector(self.create_insert_query_connector('postmeta', data_meta), True,
												   convert['id'])
		...
		product_metadata = {
			...
			'_sku': convert['sku'],
			'_regular_price': convert['price'],
			'_sale_price': convert['product_wholesale_price'],
			'_price': convert['product_wholesale_price'],
		...
		if all_meta_queries:
			self.import_multiple_data_connector(all_meta_queries, 'product')
	...
	def customer_import(self, convert, customer, customers_ext):
		...
		customer_data = {
			...
			'user_url': convert['email'],
			'user_registered': get_current_time(),
			'user_activation_key': ' ',
			'user_status': convert['username'],

		}
		customer_data_query = self.create_insert_query_connector('users', customer_data)
		customer_id = self.import_category_data_connector(customer_data_query, True, convert['id'])

		user_meta_data = {
				'user_id': customer_id,
				# 'meta_key': 'thumbnail_id',
				'meta_key': 'nickname',
				'meta_value': convert['username']
			}
		meta_query = self.create_insert_query_connector('usermeta', user_meta_data)
		self.import_data_connector(meta_query, 'customer')
	...
	def order_import(self, convert, order, orders_ext):
		...
		order_post_data = {
			...
			'post_date_gmt': get_current_time(),
			'post_content': '',
			'post_title': 'Order &ndash;'+get_current_time(),
			'post_excerpt': '',
		...
		if self._notice['config']['pre_prd']:
			order_post_data['ID'] = convert['id']
		order_post_query = self.create_insert_query_connector("posts", order_post_data)
		order_id = self.import_product_data_connector(order_post_query, True, convert['id'])
		if order_id:
		...
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		...
		order_meta_data = {
			...
			'meta_value': convert['customer']['id']
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			'post_id': order_id,
			# 'meta_key': 'thumbnail_id',
			'meta_key': '_price_include_tax',
			'meta_value': 'no'
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')

		# meta_value = convert['billing_address']['first_name'] + ' ' + convert['billing_address']['last_name'] + ' ' + convert['billing_address']['company'] + ' '+ convert['billing_address']['address_1'] + ' ' + convert['billing_address']['address_2'] + ' ' + ['billing_address']['city'] + ' ' +convert['billing_address']['state']['state_code'] + ' ' + convert['billing_address']['postcode'] + ' ' + convert['billing_address']['country']['code']+' ' + convert['billing_address']['telephone'] + ' ' + convert['customer']['email']
		order_meta_data = {
			'post_id': order_id,
			# 'meta_key': 'thumbnail_id',
		...
		# meta_value = convert['shipping_address']['first_name'] + ' ' + convert['shipping_address']['last_name'] + ' ' + \
		# 			 convert['shipping_address']['company'] + ' ' + convert['shipping_address']['address_1'] + ' ' + \
		# 			 convert['shipping_address']['address_2'] + ' ' + ['shipping_address']['city'] + ' ' + \
		# 			 convert['shipping_address']['state']['state_code'] + ' ' + convert['shipping_address'][
		# 				 'postcode'] + ' ' + convert['shipping_address']['country']['code'] + ' ' + \
		# 			 convert['billing_address']['telephone'] + ' ' + convert['customer']['email']
		order_meta_data = {
			'post_id': order_id,
			# 'meta_key': 'thumbnail_id',
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['first_name'],
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['last_name'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['company'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['address_1'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['address_2'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['city'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['state']['state_code'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['postcode'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['billing_address']['country']['code'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['customer']['email'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['customer_address']['telephone'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['first_name'],
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['last_name'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['company'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['address_1'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['address_2'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['city'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['state']['state_code'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['postcode'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			...
			'meta_value': convert['shipping_address']['country']['code'],
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		order_meta_data = {
			'post_id': order_id,
			# 'meta_key': 'thumbnail_id',
			'meta_key': '_order_restock_reduced',
			'meta_value': 'yes',
		}
		meta_query = self.create_insert_query_connector('postmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
		...
		for item in convert['items']:
			product_item_name = item['product']['name']
			order_items_data = {
			...
			order_item_query = self.create_insert_query_connector('woocommerce_order_items', order_items_data)
			order_item_id = self.import_product_data_connector(order_item_query, True, convert['id'])


			for item in convert['items']:
				product_item_qty = item['qty']
				product_id_convert = item['product']['id']
				product_id = self.get_map_field_by_src(self.TYPE_PRODUCT,product_id_convert,None)
				...
				order_product_query = self.create_insert_query_connector('wc_order_product_lookup', order_product_lookup_data)
				self.import_data_connector(order_product_query, 'order')
		...
		meta_query = self.create_insert_query_connector('woocommerce_order_itemmeta', order_meta_data)
		self.import_data_connector(meta_query, 'order')
