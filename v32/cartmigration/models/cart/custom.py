...
class LeCartCustom(LeBasecart):
	...
	def get_categories_ext_export(self, categories):
		...
		categories_ext_queries = {
			'categories': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_categories WHERE categories_id IN " + self.list_to_in_condition(
					category_ids)
			},
	...
	def convert_category_export(self, category, categories_ext):
		...
		category_data['languages']='English'
	...
	def get_products_ext_export(self, products):
		...
		product_ext_queries = {
			...
			'product_attribute':{
				'type': "select",
				'query': "SELECT * FROM _DBPRF_attribute LEFT JOIN _DBPRF_product_attribute ON _DBPRF_attribute.attribute_id = _DBPRF_product_attribute.attribute_id WHERE _DBPRF_product_attribute.product_id IN " + product_id_con ,
			}
			# 'products_meta': {
			# 	'type': "select",
	...
	def convert_product_export(self, product, products_ext):
		...
		product_data = {
			...
			'price': 0.0000,
			'product_msrp':0.0000,
			'product_wholesale_price':0.0000,
			'cost': 0.0000,
			'special_price': {
				'price': 0.0000,
				'start_date': '',
		...
		product_data['product_wholesale_price'] = product['products_wholesale_price']
		...
		product_description = get_row_from_list_by_field(products_ext['data']['products_description'], 'products_id',product['products_id'])
		...
		#
		#
		# product_images = get_list_from_list_by_field(products_ext_data['products_images'], 'products_id',product['productsid'])
		# if product_images:
		# 	if check_thumbnail:
		# 		for product_image in product_images:
		# 			image_data = get_row_from_list_by_field(products_ext_data['products_images'], 'products_id',product_image['image_id'])
		# 			product_image_data = self.construct_product_image()
		# 			if image_data :
		# 				product_image_data['url'] = url_product_image
		...
		if product_attribute:
			for product_attribute in product_attribute:
				...
				attribute_data['option_id'] = product_attribute['attribute_id']
				attribute_data['option_type'] = product_attribute['attribute_type']
				attribute_data['option_code'] = product_attribute['attribute_code']
				attribute_data['option_name'] = product_attribute['attribute_name']
				attribute_data['option_set'] = product_attribute['value']
				product_data['attributes'].append(attribute_data)

		product_data['product'] = product
		product_data['product_ext'] = products_ext
		if self._notice['config']['seo_301']:
			detect_seo = self.detect_seo()
	...
	def get_customers_ext_export(self, customers):
		...
		# country_ids = duplicate_field_value_from_list(customers['data']['address_book'], 'entry_country_id')
		# zone_ids = duplicate_field_value_from_list(customers['data']['address_book'], 'entry_zone_id')
		customer_ext_queries = {
			'customers': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_customers WHERE customers_id IN " + self.list_to_in_condition(customers_ids),
			},
			'customers_info': {
				'type': 'select',
			...
			'address_book': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_address_book WHERE address_book_id IN " + self.list_to_in_condition(
					customers_ids),
			},
			# 'countries': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM DBPRF_countries WHERE coutries_id IN  " + self.list_to_in_condition(country_ids),
			# },
			# 'states': {
			# 	'type': 'select',
	...
	def convert_customer_export(self, customer, customers_ext):
		...
		if customer['customers_gender']=='m':
			...
			customer_data['customers_gender'] = 'Female'
		...
		if address_books:
			for address_book in address_books:
				...
				address_data['company'] = customer['customers_company']
	...
	def get_orders_ext_export(self, orders):
		...
		code_cr=duplicate_field_value_from_list(orders['data'],'currency')
		# cus_ids=duplicate_field_value_from_list(orders['data'], 'cust_id')
		orders_ext_queries = {
			'orders': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_orders WHERE orders_id IN " + self.list_to_in_condition(order_ids)
			},
			'orders_products': {
				'type': 'select',
			...
			'orders_total': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_orders_total WHERE orders_total_id IN " + self.list_to_in_condition(order_ids)
			},
			'countries': {
				'type': 'select',
			...
			'currencies': {
				'type': 'select',
				'query': "SELECT * FROM  _DBPRF_currencies WHERE code IN " + self.list_to_in_condition(code_cr)
			},
			# 'address_book': {
			# 	'type': 'select',
			# 	'query': "SELECT * FROM ` _DBPRF_address_book WHERE cust_id IN " + self.list_to_in_condition(cus_ids)
			# },
	...
	def convert_order_export(self, order, orders_ext):
		...
		order_data['total']['amount'] = get_value_by_key_in_dict(order,'value'[3])
		order_data['subtotal']['title'] = 'Total products'
		order_data['subtotal']['amount'] = get_value_by_key_in_dict(order, 'value')
		...
		# order_data['currency'] = currency['title']
		# order_data['currency_value'] = order['value']
		# order_data['created_at'] = get_current_time()
		# order_data['updated_at'] = order['last_updated']
		...
		# customer_address['state']=order['delivery_state']
		...
		if customer_state:
			customer_address['state']['id'] = customer_state['zone_code']
		...
		order_billing['first_name'] = matches[0]
		order_billing['last_name'] = matches[1]
		...
		# order_delivery['country']=order['delivery_country']
		# order_delivery['state']=order['delivery_state']
		...
		if delivery_state:
			...
			order_delivery['state']['state_code'] = delivery_state['zone_id']
		...
		order_data['payment'] = order['payment_method']

		order_products = get_list_from_list_by_field(orders_ext['data']['orders_products'], 'orders_products_id', order['orders_id'])
